...
const PowerupWorld = ({ activeIndex }: { activeIndex: number }) => {
  ...
  const [rocketStart, setRocketStart] = useState<[number, number, number]>([
    0, 0, 0,
  ]);

  const isActive = Math.abs(activeIndex - 3) < 0.95;

  const { scene: bombScene } = useGLTF(ASSETS.glb.candyRed);
  const { scene: rainbowScene } = useGLTF(ASSETS.glb.candyColorFull);
  const { scene: rocketScene } = useGLTF(ASSETS.glb.candyStick);
  const { scene: shieldScene } = useGLTF(ASSETS.glb.candyGreen);
  const { scene: sparkScene } = useGLTF(ASSETS.glb.candyPink);
  ...
  const handleSelect = useCallback(
    (item: PowerupDef, position: [number, number, number]) => {
      if (item.id === "bomb") {
        burstId += 1;
