import { useProgress, Html, useGLTF, Center, Sparkles, ContactShadows, Environment } from '@react-three/drei'
...
const ALL_MODELS = [
  ...
  '/Glb-Models/Candy_prince.glb',
  '/Glb-Models/candy_pink.glb',
  '/Glb-Models/candy_model_green.glb',
  '/Glb-Models/Color_Full_Candy.glb',
  '/Glb-Models/candy_stick.glb',
  '/Glb-Models/red_candy_monster.glb'
]

const CenterPiece = () => {
  const meshRef = useRef<THREE.Group>(null)
  const { scene } = useGLTF('/Glb-Models/Mix_candy.glb')

  useFrame((state) => {
    if (meshRef.current) {
      meshRef.current.rotation.y = state.clock.getElapsedTime() * 1.5
      meshRef.current.position.y = Math.sin(state.clock.getElapsedTime() * 1.8) * 0.15
    }
  })
...
const LoaderUI = ({ progress, onComplete }: { progress: number; onComplete: () => void }) => {
    ...
    if (textOptions[index] !== flavorText) {
      gsap.to(textRef.current, { 
        ...
        onComplete: () => {
          ...
          gsap.fromTo(textRef.current, { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.4 })
        } 
      })
    }
      ...
      <div className="flex flex-col items-center justify-center w-[90vw] md:w-[480px]">
        <div className="relative w-full h-4 bg-white/5 rounded-full overflow-hidden backdrop-blur-3xl border border-white/10 p-1 shadow-2xl">
          <div 
            ...
            style={{ width: `${delayedProgress}%`, transition: 'width 0.1s linear' }}
          />
        </div>
        
        <div className="mt-10 flex flex-col items-center">
          <div ref={textRef} className="font-orbitron text-white text-xl tracking-[0.45em] uppercase font-black text-center drop-shadow-xl">
            {flavorText}
          </div>
          <span className="mt-4 font-rajdhani text-[#7ef3ff] text-base font-bold tracking-[0.25em]">{Math.round(delayedProgress)}% SWEET</span>
        </div>
      </div>
    </Html>
  )
...
const CandyLoader = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (showContent && containerRef.current) {
      gsap.fromTo(containerRef.current, { opacity: 0 }, { opacity: 1, duration: 1.5, ease: 'power2.inOut' })
    }
  }, [showContent])
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-[#0c0411] cursor-wait overflow-hidden">
      <Canvas camera={{ position: [0, 0, 8.5], fov: 42 }}>
        ...
        <hemisphereLight intensity={2.5} color="#ffe3f6" groundColor="#0c0411" />
        ...
        <Suspense fallback={null}>
          ...
          <group position={[0, -0.6, 0]}>
            <CenterPiece />
            <Sparkles count={120} scale={15} size={1.5} speed={0.4} opacity={0.3} color="#ffe28a" />
          </group>
