...
const GameExperiences = () => {
  ...
  return (
    <section ref={containerRef} className="relative w-full min-h-[120vh] bg-[#12061f] py-32 overflow-hidden flex flex-col items-center z-20">
      ...
      <div className="container-custom relative z-20 text-center mb-20 pointer-events-none">
        ...
        <h2 className="mt-6 font-orbitron text-5xl md:text-7xl font-bold text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.2)]">
          Game <span className="text-transparent bg-clip-text bg-linear-to-r from-[#76e0ff] to-[#ff74c6]">Experiences</span>
        </h2>
        ...
        <div className="flex items-center justify-center gap-8 mt-12 pointer-events-auto">
          ...
          >
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:-translate-x-1 transition-transform">
            ...
            {EXPERIENCES.map((_, i) => (
              <button 
                ...
                className={`w-4 h-4 rounded-full transition-all duration-300 cursor-pointer ${
                  activeIndex === i ? 'bg-[#76e0ff] scale-125 shadow-[0_0_15px_#76e0ff]' : 'bg-white/20 hover:bg-white/40'
                }`}
              />
            ))}
          </div>
        ...
        {EXPERIENCES.map((exp, i) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute bottom-0 left-0 right-0 p-8 z-20 transition-all duration-500 flex flex-col items-center justify-end ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 group-hover:translate-y-2 group-hover:opacity-100'}`}>
              ...
              {/* Active Glow */}
              <div className={`absolute inset-0 shadow-[inset_0_0_60px_rgba(118,224,255,0.3)] transition-opacity duration-500 z-30 pointer-events-none rounded-[2.5rem] border-2 border-[#76e0ff]/30 ${isActive ? 'opacity-100' : 'opacity-0'}`} />
              
              {/* Hover Glow (Inactive cards) */}
              <div className={`absolute inset-0 bg-white/5 transition-opacity duration-500 z-30 pointer-events-none rounded-[2.5rem] ${!isActive ? 'opacity-0 group-hover:opacity-100' : 'opacity-0'}`} />
            </div>
          )
        })}
      </div>
