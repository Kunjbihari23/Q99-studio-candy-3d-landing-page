...
useGLTF.preload('/Glb-Models/Candy_prince.glb')
...
const CandyBlob = memo(
  ({
    ...
    const { scene } = useGLTF('/Glb-Models/Candy_prince.glb')
    ...
    const updateDragTarget = useCallback((event: ThreeEvent<PointerEvent>) => {
      ...
      if (!event.ray.intersectPlane(dragPlaneRef.current, intersectionPointRef.current)) {
        return
      }
      ...
      targetStretchRef.current = THREE.MathUtils.clamp(distance / 1.6, 0, MAX_STRETCH)
    }, [])
    ...
    const releaseBlob = useCallback(() => {
      ...
      if (group) {
        group.getWorldPosition(worldCenterRef.current)
        const sparklePosition = worldCenterRef.current.clone().addScaledVector(
          releaseDirectionRef.current,
          1.05,
        )
        onRelease?.([
          sparklePosition.x,
