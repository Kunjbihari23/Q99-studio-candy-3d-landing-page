...
const resetParticleData = (data: ParticleData, count: number) => {
  for (let index = 0; index < count; index += 1) {
    ...
    data.velocities[stride] = Math.cos(azimuth) * Math.sin(elevation) * speed * spread
    data.velocities[stride + 1] = Math.cos(elevation) * speed + THREE.MathUtils.randFloat(0.6, 1.8)
    data.velocities[stride + 2] = Math.sin(azimuth) * Math.sin(elevation) * speed * spread
...
const ParticleBurst = ({
  ...
  useFrame((_, delta) => {
    ...
    for (let index = 0; index < particleCount; index += 1) {
      ...
      particleData.positions[stride] = particleData.velocities[stride] * age * damping
